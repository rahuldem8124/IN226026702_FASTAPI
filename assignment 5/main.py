...
def sort_products_by_category():
    ...
    return {
        'message': 'Sorted by category (A-Z) and price (Low to High)',
        'products': sorted_products
    }

@app.get('/products/browse')
def browse_products(
    keyword: str = Query(None, description='Word to search for'),
    sort_by: str = Query('price', description='price or name'),
    order:   str = Query('asc',   description='asc or desc'),
...
# 👈 NEW TASK: Order Pagination
@app.get('/orders/page')
def get_orders_paged(
    page:  int = Query(1, ge=1,  description='Page number'),
    limit: int = Query(3, ge=1, le=50, description='Items per page'),
):
    start = (page - 1) * limit
    end   = start + limit
    ...
    total_pages = 0 if total_orders == 0 else -(-total_orders // limit)
...
def search_orders(
    customer_name: str = Query(..., description='Name to search for')
):
    matched_orders = [
        o for o in orders 
